...
void print_help(const char *prog_name) {
    printf("Usage: %s -l <port> -d <dest> -p <port> [options]\n\n", prog_name);
    printf("Shadowport v%s - A user-space TCP tunnel and packet capture tool.\n\n", VERSION);
    printf("Required Arguments:\n");
    printf("  -l <port>       Local port to listen on.\n");
#ifdef ENABLE_DNS
    printf("  -d <host>       Destination host (IP address or hostname).\n");
#else
    printf("  -d <ip>         Destination IPv4 address.\n");
#endif
    printf("  -p <port>       Destination port.\n\n");

    printf("Optional Arguments:\n");
    printf("  -b <ip>         Bind address. Default: 127.0.0.1.\n");
    printf("  -o <file>       Output PCAP file. Default: shadowport.pcap.\n");
    printf("  --log <file>    Metadata log file (CSV format).\n");
    printf("  -q              Quiet mode (suppress console output).\n");
    printf("  -h              Show this help message.\n\n");

    printf("Purpose:\n");
    printf("  Forwards TCP traffic from a local port to a remote destination while\n");
    printf("  logging the payload to a PCAP file for analysis in Wireshark.\n");
}
...
int main(int argc, char *argv[]) {
    ...
    // Show help if no arguments provided or if -h is present
    if (argc < 2) {
        print_help(argv[0]);
        return 1;
    }
